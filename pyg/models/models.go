...
type User struct {
	...
	Addresss []*Address `orm:"reverse(many)"`
}
...
func init() {
	//注册数据库
	orm.RegisterDataBase("default", "mysql", "root:woshini88@tcp(127.0.0.1:3306)/pyg")
	//注册表结构
	orm.RegisterModel(new(User), new(Address), new(TpshopCategory), new(IndexPromotionBanner))
	orm.RegisterModel(new(IndexTypeGoodsBanner), new(IndexGoodsBanner), new(GoodsImage), new(GoodsSKU))
	orm.RegisterModel(new(GoodsType), new(Goods), new(OrderGoods), new(OrderInfo))
	//炮起来
